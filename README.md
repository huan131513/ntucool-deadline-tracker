...
![網頁面板首頁](docs/screenshots/home.png)

## 網頁導覽

網頁面板長怎樣、有哪些功能,先用畫面簡單介紹一輪(純功能介紹,不涉及技術細節;想直接動手設定的話可以跳到下面[使用說明](#使用說明快速上手))。

### 首頁總覽

上面那張圖就是打開網頁的第一個畫面:

- 右上角兩顆按鈕:**「重新整理」**去抓 Canvas 最新資料,**「發送 Telegram 通知」**手動發一次未完成作業清單
- 左側是**抓取進度**,重新整理時會即時顯示現在做到哪一步(檢查登入、連接 Canvas、抓作業…),下面是 **Canvas 連線狀態**的呼吸燈
- 右側是**小日曆**,哪天有作業/考試到期就會有顏色的小點,兩側 `‹` `›` 箭頭可以切換月份(只能在這學期的範圍內切換,超出去按鈕會變灰)

### 課程總覽

![課程卡片](docs/screenshots/courses.png)

每張卡片是一堂課,上面的「作業」「考試」數字是**還沒到期**的筆數:

- **點卡片本身** → 直接開新分頁到這堂課的 NTUCOOL 頁面
- **點卡片上「作業」「考試」的數字** → 網頁會自動捲動到下面對應的表格,不用自己找

### 作業列表

![作業列表](docs/screenshots/assignments.png)

- 每一筆顯示課程、截止時間、剩餘天數、完成狀態,**點整列**會開新分頁到 NTUCOOL 該作業的頁面
- 前面有**紅點**代表這是你還沒看過的新項目,點過一次紅點就會消失
- 橘色 **「PDF 推測」** 標籤代表這筆不是 Canvas 正式登記的作業,是系統自動讀了課程首頁附的 PDF、猜出裡面寫的截止日期——這種筆的完成狀態要自己手動點一下切換「已完成/未完成」,因為沒有 Canvas 資料可以查
- 下面的**「課程首頁可能提到的作業」**是低信度提示:從課程首頁或課程大綱掃到「作業」相關字眼,但不確定是不是真的有作業,列出來提醒你自己點進去看一下

### 考試列表

![考試列表](docs/screenshots/exams.png)

- 跟作業列表邏輯一樣,多了一個小標籤顯示這是「測驗」還是課程行事曆上的考試
- 下面的**「公告中可能提到的考試」**也是低信度提示:從公告欄掃到跟考試相關的字眼(例如「期中」「期末」),列出來但不保證正確,一樣要自己點進去確認
